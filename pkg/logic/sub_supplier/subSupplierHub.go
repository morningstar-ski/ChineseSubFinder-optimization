...
func (d *SubSupplierHub) downloadMovieSubFromSuppliers(videoFullPath string, index int64, suppliers []ifaces.ISupplier) ([]string, error) {
	if len(suppliers) < 1 {
		return nil, nil
	}

	subInfos := movieHelper.OneMovieDlSubInAllSite(d.log, suppliers, videoFullPath, index)
	if len(subInfos) < 1 {
...
func (d *SubSupplierHub) dlSubFromSeriesInfo(seriesDirPath string, index int64, seriesInfo *series.SeriesInfo, suppliers []ifaces.ISupplier) (map[string][]string, error) {
	if len(suppliers) < 1 {
		return nil, nil
	}

	subInfos := seriesHelper.DownloadSubtitleInAllSiteByOneSeries(d.log, suppliers, seriesInfo, index)
	if len(subInfos) < 1 {
